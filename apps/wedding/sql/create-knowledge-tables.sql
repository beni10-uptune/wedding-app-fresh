...
CREATE INDEX idx_knowledge_embedding ON wedding_knowledge USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100);
...
CREATE TABLE IF NOT EXISTS wedding_preferences_learned (
  wedding_id UUID PRIMARY KEY REFERENCES weddings(id),
...
-- Create function for BPM progression lookup
CREATE OR REPLACE FUNCTION get_bpm_for_time(
  current_time TIME
)
RETURNS TABLE (
  phase TEXT,
...
BEGIN
  ...
  SELECT 
    ...
    p.energy_level,
    p.mixing_technique
  FROM bpm_progression p
  WHERE current_time BETWEEN 
    p.time_range::TIME AND 
    (p.time_range::TIME + INTERVAL '1 hour')
