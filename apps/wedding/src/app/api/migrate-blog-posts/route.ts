...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('Migrating blog posts from blog_posts to blogPosts...')
    
    // First, migrate the author
    const authorId = 'uptune-music-team'
    await adminDb.collection('blogAuthors').doc(authorId).set({
    ...
    return NextResponse.json({
      message: 'Blog posts migrated to camelCase collections',
      results,
    ...
    return NextResponse.json(
      { error: 'Failed to migrate blog posts', details: String(error) },
      { status: 500 }
    )
  }
}